...
def compute_coefficients(stack_def_file, magic_startup_file=None, doshield=True, dopartial=True, dosidewall=True, verbose=0):
    ...
    if verbose:
        print(f"magic_startup_file: {magic_startup_file}")
        print(f"doshield: {doshield}")
        print(f"dopartial: {dopartial}")
        print(f"dosidewall: {dosidewall}")
    ...
    if verbose:
        ...
        print(f"feature_size: {stackupfile_dict['feature_size']}")
        print(f"process: {stackupfile_dict['process']}")
    ...
    if dosidewall and doshield and dopartial:
        # Make sure the magic directory exists
        os.makedirs(os.path.join(process, 'magic'), exist_ok=True)

        # Write dummy magicrc file
        startupfile = os.path.abspath(os.path.join(process, 'magic', f'{process}.magicrc'))
        
        print(f"Overriding the magic startupfile with the generated one: {startupfile}.")
        
        with open(startupfile, 'w') as magicrc_file:
...
""")
        ...
        print("The magic techfile won't be generated because one of dosidewall, doshield or dopartial is false.")
